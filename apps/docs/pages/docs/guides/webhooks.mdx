...
```ts filename="app/api/routes.ts" copy
...
[creating a test request](../api/node#testing-webhook-signing). In addition to
that, you can also check the `Logs and events` on the payment intent page of
your server to see if the webhook event was received.

## List of webhook events available

- `payment_intent.succeeded` - Occurs whenever a new PaymentIntent is
  successfully created. See the Payment Intents API for more details.
